...
function initializeMap() {
    let zoom = 5
    let center = new BMapGL.Point(104.366293, 38.066465);
...
function updateHexgaons(embedImg) {

    let ctx = window.parent.dwf_ctx;
    let hexagons = [];

    if (ctx) {
        let addin = ctx.getAddinById("FormEngine1");
        if (addin) {
            hexagons = addin.getSelected();
            if (hexagons.length == 0) {
                hexagons = addin.getAll();
            }
        }
    }
    ...
    if (hexagons && hexagons.length > 0) {
        let zoom = 9;
        let center = findCenter(hexagons);
        if (embedImg) {
            embedHexagons(hexagons);
        }
        else {
...
function findCenter(hexagons){
    if (hexagons && hexagons.length > 0) {
        let lng = hexagons.map((h) => (h.HexVertex1LON + h.HexVertex4LON) / 2).reduce((c, h) => c += h)
        let lat = hexagons.map((h) => (h.HexVertex1LAT + h.HexVertex4LAT) / 2).reduce((c, h) => c += h)
        console.log(lng, lat);
        return new BMapGL.Point(lng/hexagons.length, lat/hexagons.length);
    }
}
...
function searchInPolygon() {
    ...
    if (ctx) {
        ...
        let bounds = curOverlay.getBounds();
        let sw = bounds.getSouthWest();
        let ne = bounds.getNorthEast();

        let query = `and obj.HexVertex1LON > ${sw.lng} and obj.HexVertex6LON < ${ne.lng} and obj.HexVertex1LAT > ${sw.lat} and obj.HexVertex6LAT < ${ne.lat} order by obj.DATE_ACQUIRED desc, obj.CORNER_UL_LON, obj.CORNER_UL_LAT limit 100`;

        ctx.handleQueryData({ query: query, targetClass: "Hexagon" }, true).then(hexagons => {
            var newHexagons = new Array();
            hexagons.forEach(h => {
                var point = new BMapGL.Point((h.HexVertex1LON + h.HexVertex4LON) / 2,(h.HexVertex1LAT + h.HexVertex4LAT) / 2);
                var isInPolygon = BMapGLLib.GeoUtils.isPointInPolygon(point, curOverlay);
            ...
            drawHexagons(newHexagons);
            if (newHexagons.length > 0) {
               map.panTo(findCenter(newHexagons));
            }
        });
...
function searchInDistrict() {
    let ctx = window.parent.dwf_ctx;
    if (ctx) {
        
        var district = prompt("请输入行政区域","北京市海淀区");
        if (district != null & district != ""){
		    ...
		    bdary.get(district, function(rs){       //获取行政区域
          	...
          	var pointArray = [];
          	var ply = new BMapGL.Polygon(rs.boundaries[0], {strokeWeight: 2, strokeColor: "#ff0000"}); //建立多边形覆盖物
			map.addOverlay(ply);  //添加覆盖物
			pointArray = pointArray.concat(ply.getPath());
			map.setViewport(pointArray);    //调整视野  
			
			
			let bounds = ply.getBounds();
            let sw = bounds.getSouthWest();
            let ne = bounds.getNorthEast();
...
function drawHexagons(hexagons) {
    if (hexagons && hexagons.length > 0) {
        hexagons.forEach(h => {
            ...
            coords.push(new BMapGL.Point(h.HexVertex1LON, h.HexVertex1LAT));
            coords.push(new BMapGL.Point(h.HexVertex2LON, h.HexVertex2LAT));
            coords.push(new BMapGL.Point(h.HexVertex3LON, h.HexVertex3LAT));
            coords.push(new BMapGL.Point(h.HexVertex4LON, h.HexVertex4LAT));
            coords.push(new BMapGL.Point(h.HexVertex5LON, h.HexVertex5LAT));
            coords.push(new BMapGL.Point(h.HexVertex6LON, h.HexVertex6LAT));

            let hexLayer = new BMapGL.Polygon(coords, { strokeColor: "blue", strokeWeight: 2, strokeOpacity: 0.5 });

            hexLayer.addEventListener("click", function() {
                console.log(1234);
            });
            
            map.addOverlay(hexLayer);           //增加多边形

            // let marker = new BMapGL.Marker(center);
            // marker.setTitle(h.Code);
            // let center = new BMapGL.Point((h.HexVertex1LON + h.HexVertex4LON) / 2, (h.HexVertex1LAT + h.HexVertex4LAT) / 2);            
            ...
            //         conditionExpre: `and obj.oid = '${h.oid}'`
            //     };
            //     console.log(opr);
            //     let ctx = window.parent.dwf_ctx;
            //     ctx.executeOperation(opr);
            // });
            // map.addOverlay(marker);
        });
    }
...
function embedHexagons(hexagons) {
    let ctx = window.parent.dwf_ctx;
    if (hexagons && hexagons.length > 0) {
        hexagons.forEach(h => {
            let offset = h.CORNER_UL_LAT - h.CORNER_LR_LAT;
            let ul = new BMapGL.Point(h.CORNER_UL_LON, h.CORNER_UL_LAT + offset);
            let lr = new BMapGL.Point(h.CORNER_LR_LON, h.CORNER_LR_LAT + offset);
            console.log('ul',ul,'lr',lr);
            let hexBounds = new BMapGL.Bounds(ul, lr);
            let opt = {
                type: 'image',
                opacity: 1,
                url: `http://${ctx.env.serverIp}:${this.env.serverPort}/dwf/v1/omf/classes/Hexagon/objects/${h.oid}/attributes/Thumbnail/bytes?attachment=true&r=${Math.random()}`
            }
            console.log(opt.url);
            let hexLayer = new BMapGL.GroundOverlay(hexBounds, opt);
            map.addOverlay(hexLayer);
        });
    }
}
...
function addMapControl() {
    // 通过JavaScript的prototype属性继承于BMapGL.Control
    ...
    ZoomControl.prototype.initialize = function (map) {
        ...
        p.appendChild(document.createTextNode("查看"));
        p.onclick = (e) => {
            map.clearOverlays();
            updateHexgaons(false);
        }
        div.appendChild(p);
        // 添加清空
        p = document.createElement("button");
        p.appendChild(document.createTextNode("清空"));
        p.onclick = (e) => {
            map.clearOverlays();
        }
        div.appendChild(p);
        // 添加还原
        p = document.createElement("button");
        p.appendChild(document.createTextNode("还原"));
        p.onclick = (e) => {
            map.clearOverlays();
            let center = new BMapGL.Point(104.366293, 38.066465);
        ...
        // 添加检索
        p = document.createElement("button");
        p.appendChild(document.createTextNode("搜索"));
        p.onclick = (e) => {
            map.clearOverlays();
            searchInBounds();
        }
        ...
        // 添加多边形检索
        p = document.createElement("button");
        p.appendChild(document.createTextNode("多边形搜索"));
        p.onclick = (e) => {
            map.clearOverlays();
            searchInPolygon();
        }
        ...
        // 添加行政区域检索
        p = document.createElement("button");
        p.appendChild(document.createTextNode("行政区域搜索"));
        p.onclick = (e) => {
            map.clearOverlays();
            searchInDistrict();
        }
        ...
        // 添加镶嵌
        p = document.createElement("button");
        p.appendChild(document.createTextNode("镶嵌"));
        p.onclick = (e) => {
            map.clearOverlays();
            //</br></br>map.setMapType(BMAP_NORMAL_MAP);
            updateHexgaons(true);
        }
        div.appendChild(p);
        // 设置样式
